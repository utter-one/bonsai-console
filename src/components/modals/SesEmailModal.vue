...
const sesProviders = computed(() =>
  providersStore.items.filter(p => p.apiType === 'ses')
)
...
<template>
  <BaseModal title="Send Email (SES)" size="md" @close="$emit('close')">
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
      Send an outgoing email via Amazon SES. The conversation record is created immediately. Future replies from the recipient will be attached to the same conversation.
    </p>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <FormField label="Channel Provider" path="channelProviderId" :error="null" required class="w-full">
        <select v-model="form.channelProviderId" class="form-select">
          <option value="" disabled>Select an SES provider</option>
          <option v-for="p in sesProviders" :key="p.id" :value="p.id">{{ p.name }}</option>
        </select>
      </FormField>
