...
- **Settings** — retention in days (minimum 7) for call logs / health checks / metric samples, the provider health probe policy (LLM probe mode — note that `one_token` probes cost money — ASR/TTS probe modes, and probe cooldown), and the alert engine settings (evaluation interval and default re-fire cooldown).
