...
function handleClickOutside(event: MouseEvent) {
  ...
  const dropdown = document.querySelector('.latency-time-dropdown')
  const button = document.querySelector('.latency-time-button')
  ...
  <div class="mb-6 flex flex-wrap items-center gap-3">
    <div class="relative">
      <button
        ...
        class="latency-time-button filter-btn !shadow-none"
      >
        <Calendar class="w-4 h-4 mr-2" />
        <span>{{ currentTimeFilterLabel }}</span>
        <ChevronDown class="w-4 h-4 ml-2" />
      </button>
      <div v-if="showTimeDropdown" class="latency-time-dropdown filter-dropdown-panel min-w-[200px]">
        <button
