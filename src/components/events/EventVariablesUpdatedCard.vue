...
import { ref } from 'vue'
import { Braces, ChevronRight, ChevronDown } from 'lucide-vue-next'
...
<template>
  <div class="grid grid-cols-[auto_auto_1fr] gap-x-2">
    ...
    <Braces class="place-self-center w-5 h-5 text-emerald-600" />
    <div style="display:contents">
      <div class="min-w-0 flex items-center gap-2">
        <button @click="expanded = !expanded" class="font-semibold text-emerald-900 dark:text-emerald-100 shrink-0 text-left">Variables Updated</button>
        ...
        <div v-if="event.eventData.variables && Object.keys(event.eventData.variables).length > 0">
          <details class="group" open>
            ...
            <div class="mt-1 bg-white bg-opacity-60 rounded p-2 font-mono text-xs overflow-x-auto dark:bg-gray-900 dark:bg-opacity-60">
              <pre class="whitespace-pre-wrap wrap-break-word dark:text-gray-300">{{ JSON.stringify(event.eventData.variables, null, 2) }}</pre>
            </div>
          </details>
        </div>
