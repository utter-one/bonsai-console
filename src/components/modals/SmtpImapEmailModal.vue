...
const smtpImapProviders = computed(() =>
  providersStore.items.filter(p => p.apiType === 'smtp_imap')
)
...
async function handleSubmit() {
  ...
  try {
    const conversationId = await conversationsStore.initiateSmtpImap({
...
<template>
  <BaseModal title="Send Email (SMTP/IMAP)" size="md" @close="$emit('close')">
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
      Send an outgoing email via SMTP/IMAP. The conversation record is created immediately. Future replies from the recipient will be picked up by the IMAP polling service.
    </p>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <FormField label="Channel Provider" path="channelProviderId" :error="null" required class="w-full">
        <select v-model="form.channelProviderId" class="form-select">
          <option value="" disabled>Select an SMTP/IMAP provider</option>
          <option v-for="p in smtpImapProviders" :key="p.id" :value="p.id">{{ p.name }}</option>
        </select>
      </FormField>
