...
import { useAnalyticsStore, useAuthStore, useStagesStore, useGlobalActionsStore, useGuardrailsStore, useProjectsStore, useToolsStore } from '@/stores'
...
async function renameActiveQuery() {
  ...
  saveError.value = null
  try {
    const updated = await analyticsStore.updateSavedFunnelQuery(props.projectId, activeQuery.value.id, {
...
async function saveAsNew() {
  ...
  saveError.value = null
  try {
    const created = await analyticsStore.createSavedFunnelQuery(props.projectId, {
...
async function deleteActiveQuery() {
  ...
  saveError.value = null
  try {
    await analyticsStore.deleteSavedFunnelQuery(props.projectId, activeQuery.value.id, activeQuery.value.version)
    activeQuery.value = null
    showSaveMenu.value = false
  } catch (err: any) {
...
onMounted(async () => {
  ...
  if (props.projectId) {
    await loadProjectData(props.projectId)
  }
      ...
      <div ref="querySelectorRef" class="relative items-center hidden sm:flex gap-2 flex-1 min-w-0">
        ...
        >
          ...
          <div class="border-t border-gray-100 dark:border-gray-700 mt-1 pt-1">
            <button
              @click="analyticsStore.fetchSavedFunnelQueries(props.projectId); showQuerySelector = false"
