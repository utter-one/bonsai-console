...
const iconColor = computed(() =>
  isBlocking.value ? 'text-red-600' : 'text-amber-500',
)
</script>
...
<template>
  <div class="flex items-start gap-2">
    ...
    <ShieldAlert class="w-5 h-5 mt-0.5 shrink-0" :class="iconColor" />
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-2" :class="{ 'mb-2': expanded }">
        ...
        <span v-if="isBlocking" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 border border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800 shrink-0">blocked</span>
        <span v-if="!expanded && event.eventData.blockingCategories?.length" class="text-xs text-gray-500 truncate">{{ event.eventData.blockingCategories.join(', ') }}</span>
      ...
      <div v-show="expanded" class="space-y-2">
        ...
        <div v-if="event.eventData.input">
          ...
          <div class="text-sm text-gray-900 dark:text-gray-200 mt-1">{{ event.eventData.input }}</div>
        </div>
