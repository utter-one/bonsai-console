...
const voiceProviders = computed(() =>
  providersStore.items.filter(p => p.apiType === 'twilio_voice')
)
...
async function handleSubmit() {
  ...
  try {
    const conversationId = await conversationsStore.initiateVoiceCall({
    ...
    submitError.value = err.response?.data?.message || 'Failed to initiate call'
  } finally {
...
<template>
  <BaseModal title="Initiate Voice Call" size="md" @close="$emit('close')">
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
      Place an outgoing Twilio Voice call. The conversation record is created immediately and the call is connected asynchronously when the recipient answers.
    </p>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <FormField label="Channel Provider" path="channelProviderId" :error="null" required class="w-full">
        <select v-model="form.channelProviderId" class="form-select">
          <option value="" disabled>Select a Twilio Voice provider</option>
          <option v-for="p in voiceProviders" :key="p.id" :value="p.id">{{ p.name }}</option>
        </select>
      </FormField>
    ...
    <template #footer>
      <div class="modal-footer">
        ...
        <button type="button" class="btn-primary" :disabled="!isValid || isSubmitting" @click="handleSubmit">
          {{ isSubmitting ? 'Initiating...' : 'Initiate Call' }}
        </button>
      </div>
    </template>
  </BaseModal>
